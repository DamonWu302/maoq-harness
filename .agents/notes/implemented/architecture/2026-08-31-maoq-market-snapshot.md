...
`@deepseek-ai/dsh-market-snapshot-mysql` is the first production acquisition adapter. It reuses the quality-gated `long_short_stock` database through parameterized, session-read-only queries instead of duplicating upstream Tushare requests. It discovers the exact quality, price, adjustment, basic, price-limit, index, and SW L1 versions; verifies them before and after acquisition; requires the joined daily population to equal the quality decision; and derives only deterministic sector, breadth, and emotion facts. It remains opt-in because database location and credentials are deployment facts.
