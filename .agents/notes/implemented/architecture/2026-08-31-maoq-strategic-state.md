...
`maoq_analyze_strategy` loads snapshots by exact hash, supplies only deterministic features to selected specialists, requires evidence and method references in every report and synthesis, and runs an independent fresh risk review. Every child output schema enumerates the evidence addresses from that exact feature record, preventing an unknown reference from invalidating the completed council after all child calls have run; host validation remains the final fail-closed boundary. The host preserves deterministic labels, resolves attributions, and combines validated posture with the risk verdict. The lower-level `maoq_decide` remains available for council-runtime diagnostics and is not the P2 strategic decision path.
...
The same ordered snapshot hashes and engine version produce byte-identical deterministic features; prompt or model changes can affect only the separately stored interpretation. Every accepted interpretation names supporting and opposing snapshot fields, falsifiable transition conditions, confidence, and at least one sourced method application. Missing history, stale inputs, or invented citations fail closed. Thresholds are explicit engine policy: changing them requires a version bump and updated gold fixtures. The fixed attribution catalog improves auditability but deliberately gives up verbatim quotation and edition page references. Intraday state remains outside this daily contract.
