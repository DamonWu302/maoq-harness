...
`maoq_analyze_strategy` loads snapshots by exact hash, supplies only deterministic features to selected specialists, requires evidence and method references in every report and synthesis, and runs an independent fresh risk review. The host preserves deterministic labels, resolves attributions, and combines validated posture with the risk verdict. The lower-level `maoq_decide` remains available for council-runtime diagnostics and is not the P2 strategic decision path.
