...
- **Static instance selection** — Profile rows fix provider names and tool bindings; settings may change the model used by later runs, but calls cannot select a provider or model, and every exposed tool needs a unique `toolName`.
