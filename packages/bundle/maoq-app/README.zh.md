...
MAOQ Profile 还会挂载专属浏览器工具行，用“查看快照目录”“生成交易日快照”“读取最新市场战略状态”“MAOQ 战略研判”等业务名称替代通用的技术工具调用回退。已完成的战略决策会持久化到 `.maoq/decisions`；完全相同的重复分析输入以及最新／历史查询会复用这些镜像，不再启动新的议事组子 Agent。
