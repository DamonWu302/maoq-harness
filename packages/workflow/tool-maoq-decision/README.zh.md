...
编排脚本、结构定义、提供者路由和子 Agent 上限均由部署拥有。战略路径在任何子 Agent 运行前按精确哈希加载快照并计算带版本特征。所选专家通过 `Promise.all` 并行运行，综合与风险审查随后由相互独立的新 Agent 完成。宿主会拒绝角色漂移、改写确定性标签、未知证据引用、未识别方法 ID、矛盾风险字段，以及任何让过期或残缺输入变得可行动的尝试。
