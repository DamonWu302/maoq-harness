...
- **每日生成仍需手动或外部调度** — 规范化工具本身是幂等的，但本包尚未自行启动收盘后或事件驱动调度器。
