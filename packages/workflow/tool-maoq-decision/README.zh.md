...
最新与按 ID 查询工具会在不修改镜像的前提下判断当前可用性。它们从宿主目录自动解析在截止时间前最新的快照，不信任模型传入的哈希。超过最大时效、快照无法核验或已经改变、特征／工作流版本漂移、分析模式改变或提供方路由改变，都会返回 `freshness.status: stale`、`currentUseAllowed: false` 和明确原因。记录仍可用于回放，但不能悄悄变成当前建议。
