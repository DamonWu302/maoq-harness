...
function defensiveDecision(input: ResearchTacticSuiteHistoryEvaluation) {
  ...
  return createTacticCommanderDecision(route, {
    ...
    specialistReports: ['big_bull_trend', 'short_sentiment'].map(role => ({
      ...
      analysis: 'The current route lacks sufficient payoff asymmetry.',
      ...
      confidence: 0.6,
      invalidationConditions: ['One routed tactic gains a stronger evidence margin.'],
    })),
    ...
    quantRouteAssessment: 'The routed evidence does not justify attack.',
