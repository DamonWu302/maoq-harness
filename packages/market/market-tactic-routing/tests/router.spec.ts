...
describe('deterministic tactic routing', () => {
  it('routes qualified research tactics by score while preserving zero paper scope', () => {
    ...
    expect(route.advisoryUniverse).toHaveLength(11)
    expect(route.advisoryUniverse.filter(item => item.quantDisposition === 'top_three').map(item => item.tacticId))
      .toEqual(route.slate.map(item => item.tacticId))
    expect(route.advisoryUniverse.find(item => item.tacticId === 'platform_consolidation_second_advance'))
      .toMatchObject({ quantDisposition: 'rejected', quantReasons: ['missing_conditional_record'] })
