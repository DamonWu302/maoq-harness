...
`dsh-market-tactic-routing` 把已经结束的战法结果归因到原决策截止点可知的战略事实，持久化不可变结果与聚合代际，并从一份有界战绩中路由当前合格战法。它把前三保留为量化建议，把所有通过硬门槛的目录战法暴露为有界参谋全集，并把统帅完整战法池提案与独立否决校验并持久化为一份可回放决策。确定性路由器不使用模型；主动证据不足时，`defensive_no_trade` 始终可用。
...
成功时会返回不可变且带内容地址的记录。未来可见结果、不兼容战法版本、未推进的截止点、不匹配的资格记录、残缺战略事实、晚于决策截止点的战绩、硬可行参谋全集外战法、缺少反证的量化推翻或矛盾的否决都会失败关闭。量化前三之外的战法保持研究范围和零模拟权限。`TacticRoutingStore` 按 UTC 可用日期发布结果，并按内容身份发布战绩、名单与统帅决策；有界区间读取只取得两个战绩截止点之间的分区。
...
v3 路由器选择达到八个成熟样本的最窄证据层：精确环境，其次是市场状态加情绪周期，最后是同一市场状态。它绝不跨市场状态借用证据。硬可行战法即使位于目录偏好状态之外也会参与竞争；市场与情绪适配只贡献有界的 0.15、0.08、0.04 或零先验。合并后的收益、风险和执行指标依据充分统计量重新计算；近期有效性按各单元样本数加权。证据层越宽，环境对齐得分越低。主动战法仍然需要为正的 95% 期望下界、为正的翻倍成本期望、不低于 50% 的成交率，以及为正的最终得分。研究战法可以进入研究名单，但模拟仓位上限仍为零。

带版本的转移选择器会在主动候选合格时从防守立即进入，并在当前战法从名单消失时立即退出。其他自主切换必须先持有当前主动战法至少五个可路由交易日，而且挑战者得分至少高出 0.03。带内容地址的记录暴露原战法、挑战者、所选战法、持有日数、分数优势与原因。日级回放同时报告原始无状态路由和转移控制路由，使切换成本能够单独归因。
...
- **转移证据仍属于开发阶段**——2022—2025 年回放只触发一次最短持有覆盖，没有触发分数优势切换，因此固定的五日和 0.03 参数仍需封存留出集验证。
