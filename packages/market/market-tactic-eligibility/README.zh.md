...
目录包含六项主动研究战法：状态签名突破回踩、可成交情绪龙头、行业相对超跌修复、相关性聚类板块轮动、板块残差强势和低波板块领涨，并包含防守／空仓。每项定义拥有自己的战法家族，以及明确的偏好市场状态与情绪周期覆盖。所有硬可行的主动战法均有意保持 `research_only`；偏好状态不匹配会作为软环境证据保留，绝不会让它晋级为 `eligible`。
