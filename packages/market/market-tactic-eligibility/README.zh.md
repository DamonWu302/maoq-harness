...
目录包含六项主动研究战法：状态签名突破回踩、可成交情绪龙头、行业相对超跌修复、相关性聚类板块轮动、板块残差强势和低波板块领涨，并包含防守／空仓。每项定义拥有自己的战法家族，以及明确的市场状态与情绪周期覆盖。全部主动战法均有意保持 `research`；即使状态匹配，也只能产生 `research_only`，绝不会产生 `eligible`。
...
每个主动战法首先要求市场状态、情绪周期和板块战场组件均已就绪。随后，由目录拥有的状态与情绪覆盖约束适用环境；只有排名第一的板块得分为正，结果才会暴露板块 ID。自动化测试会检查每种市场状态均有主动战法覆盖，避免目录遗漏让某种状态天然只能空仓。结果将状态适配与晋级状态分开，因此模型叙事不能让研究项目晋级。证据缺失会让主动战法失败关闭，同时继续保留防守选项。
