...
目录包含十项主动研究战法：状态签名突破回踩、平台整理二次上攻、AH52 阻力路径、可成交情绪龙头、核心首次分歧修复、首板延迟价格发现、行业相对超跌修复、相关性聚类板块轮动、板块残差强势和低波板块领涨，并包含防守／空仓。每项定义拥有自己的战法家族，以及明确的偏好市场状态与情绪周期覆盖。所有硬可行的主动战法均有意保持 `research_only`；偏好状态不匹配会作为软环境证据保留，绝不会让它晋级为 `eligible`。
...
每个主动战法首先要求市场状态、情绪周期和板块战场组件均已就绪，并且排名第一的板块得分为正；这些属于硬可行性门禁。目录拥有的状态与情绪覆盖属于软偏好先验：不匹配时 `contextFit` 保持 false，并产生稳定的 `STATE_MISMATCH` 原因，但不会排除其他方面可行的研究候选。是否具有足够强的同状态成熟证据来与防守竞争，由路由器而非本评估器决定。结果将状态适配与晋级状态分开，因此模型叙事不能让研究项目晋级。硬证据缺失会让主动战法失败关闭，同时继续保留防守选项。
