...
const DEFINITIONS: readonly TacticDefinition[] = deepFreeze([
  ...
  {
    tacticId: 'platform_consolidation_second_advance',
    tacticVersion: 'platform-consolidation-second-advance-v1',
    family: 'trend',
    eligibleMarketRegimes: ['risk_on_trend', 'rotation', 'repair'],
    eligibleEmotionCycles: ['startup', 'acceleration', 'repair'],
    promotionStatus: 'research',
    evidenceGrade: 'B',
    ...
    entryPolicy: ['intact positive medium trend', 'controlled platform with contracting supply and renewed relative strength'],
    exitPolicy: ['exit on platform support loss', 'time exit when the second advance does not propagate'],
    invalidationPolicy: ['mainline leadership changes', 'platform breaks on expanding turnover', 'sector breadth collapses'],
    executionRequirements: ['T+1', 'point-in-time platform measurement', 'next-open cost and capacity model'],
  },
  {
    tacticId: 'ah52_resistance_path',
    tacticVersion: 'ah52-resistance-path-v1',
    family: 'trend',
    eligibleMarketRegimes: ['risk_on_trend', 'rotation'],
    eligibleEmotionCycles: ['startup', 'acceleration', 'repair'],
    promotionStatus: 'research',
    ...
    maximumHoldingSessions: 20,
    maximumPaperPositionPct: 8,
    entryPolicy: ['positive market and sector trend', 'price approaches the 252-session high through shrinking overhead resistance'],
    exitPolicy: ['exit on failed high reclaim', 'reduce when sector breadth or relative strength reverses'],
    invalidationPolicy: ['overhead supply expands', '252-session high attempt fails', 'market trend contracts'],
    executionRequirements: ['T+1', '252-session adjusted history', 'next-open cost and capacity model'],
  },
  {
  ...
  {
    tacticId: 'first_divergence_core_repair',
    tacticVersion: 'first-divergence-core-repair-v1',
    family: 'emotion',
    eligibleMarketRegimes: ['risk_on_trend', 'rotation', 'repair'],
    eligibleEmotionCycles: ['divergence', 'repair', 'acceleration'],
    promotionStatus: 'research',
    evidenceGrade: 'B',
    requiredHistorySessions: 20,
    maximumHoldingSessions: 5,
    maximumPaperPositionPct: 6,
    entryPolicy: ['first controlled divergence inside an intact mainline', 'core leadership, liquidity, and next-session executability are mandatory'],
    exitPolicy: ['exit when repair fails or the core loses leadership', 'no averaging after renewed divergence'],
    invalidationPolicy: ['mainline breadth breaks', 'loss effect expands', 'core status is lost'],
    executionRequirements: ['T+1', 'no sealed-limit fill assumption', 'short-horizon slippage and capacity model'],
  },
  {
    tacticId: 'first_limit_delayed_price_discovery',
    tacticVersion: 'first-limit-delayed-price-discovery-v1',
    family: 'emotion',
    eligibleMarketRegimes: ['risk_on_trend', 'rotation'],
    eligibleEmotionCycles: ['startup', 'acceleration'],
    promotionStatus: 'research',
    evidenceGrade: 'B',
    requiredHistorySessions: 20,
    maximumHoldingSessions: 3,
    maximumPaperPositionPct: 5,
    entryPolicy: ['broad sector confirmation around an executable first limit-up', 'enter only through next-session price discovery'],
    exitPolicy: ['fast exit when delayed discovery fails', 'time exit after three sessions'],
    invalidationPolicy: ['isolated first limit-up', 'sector breadth fails', 'next-session price is not executable'],
    executionRequirements: ['T+1', 'board-specific daily-limit reconstruction', 'queue, gap, cost, and capacity model'],
  },
  {
