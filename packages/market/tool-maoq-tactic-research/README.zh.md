...
`maoq_tactic_research_sources` 无需扫描数据库，即可列出已注册历史来源、全部六项固定战法版本和经过审计的公开基准。每项基准都保留样本、组合频率、报告夏普、决策用途与不可比限制，且都不能改变晋级状态。`maoq_tactic_backtest` 接收一个获准来源、一项固定战法 id 和一个闭区间日期范围。工具强制执行部署所有的质量与范围上限，只以流式方式读取所选适配器一次，并返回数量、绩效统计、按时间折、最近非空候选信号、内容哈希与晋级阻断项，而不是完整权益曲线或全部市场行。
