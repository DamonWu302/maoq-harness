...
`LongShortStockTacticHistoryAdapter` 只有在上游质量行明确标记为 `complete`、可用且行数一致时，才按调用方限定的块大小流式读取闭区间日期。每一条来源价格行都必须保留在复权、换手和涨跌停必需连接结果中。为避免全区间 SQL 窗口工作，它先一次取得区间前个股收盘价，随数据流推进昨收映射，并只取得与当前分块重叠的成员关系期间，再在内存中附加最新有效的申万一级成员关系。每个日期必须包含上证综指、沪深 300、中证 500 与中证 1000 的精确收益；适配器还会推导全市场等权基准，并在必需指数缺失时失败关闭。每个块把后复权特征交易时段与包含精确涨跌停价的独立原始行情配对，绑定来源与映射版本，并生成稳定内容哈希。适配器只读并采用流式分块；它既不排序股票，也不把完整日期范围保存在内存中。
