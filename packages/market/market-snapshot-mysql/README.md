...
`LongShortStockTacticHistoryAdapter` reads an inclusive date range only when the upstream quality row explicitly says `complete`, usable, and row-count consistent, then streams it in caller-bounded chunks. Every source price row must survive the required adjustment, turnover, and price-limit joins. To avoid range-wide SQL window work, it fetches pre-range stock closes once, advances that prior-close map with the stream, and loads only membership periods overlapping each chunk before attaching the latest effective SW L1 membership in memory. Every date must contain exact returns for SSE Composite, CSI 300, CSI 500, and CSI 1000; the adapter also derives a full-universe equal-weight benchmark and fails closed when a required index is absent. Each chunk pairs HFQ feature sessions with separate raw bars and exact price limits, binds source and mapping versions, and receives a stable content hash. The adapter is read-only and streams chunks; it neither ranks stocks nor retains the complete range in memory.
