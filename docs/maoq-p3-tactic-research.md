...
### Expanded evidence screen

This round searches for distinct return sources instead of renaming and retuning breakout, limit-up, and oversold rules. Public results only prioritize research: most cited portfolios are monthly or long-short and are not MAOQ long-only executable returns.

| Tactic family | Public evidence | Daily/sector fit | Treatment |
|---|---|---|---|
| Correlation-cluster sector rotation | [Boubaker, Du, and Liu](https://doi.org/10.1057/s41260-021-00248-8) study Shanghai and Shenzhen A-shares from 2007–2020 and report that consolidating highly correlated industries raises industry-momentum Sharpe from 0.71 to 1.16; their reported optimum correlation is 0.75. | Requires sector daily returns, correlations, breadth, and stock mapping; weekly decisions can contain turnover. | **Priority 1.** Freeze one clustering method and one predeclared threshold; never sweep around 0.75. Trade only long sector exposure, not the paper's long-short legs. |
| Market/sector residual strength | [Chinese residual-momentum evidence](https://doi.org/10.1016/j.frl.2018.12.014) reports a 10.584% annual winner-minus-loser return over 1997–2017 with lower extreme risk than conventional momentum; [another China IMOM study](https://sites.uwasa.fi/mfjuva/wp-content/blogs.dir/4/files/sites/231/2025/02/MJ0p1e5ukb87jckffrsgmslfr1mh24.pdf) also reports significant improvement over conventional momentum. | Rolling daily-return regressions can remove market and Shenwan-industry returns without fundamentals. | **Priority 2.** Buy positive residual strength inside strong sectors, not the raw market return leaderboard; residual volatility, liquidity, and price-limit gates are mandatory. |
| Low-volatility sector leader | [The Volatility Effect in China](https://assets.ctfassets.net/tl4x668xzide/1QBgySFzYmwqGlDmSaFYay/b118ecac9f394202b5d657f33f5d5fab/the-volatility-effect-in-china.pdf) reports 9.1% annual return, 10.3% volatility, and 0.88 Sharpe for the A-share low-risk factor from 2000–2018, versus -0.07 Sharpe for conventional momentum. | Needs only daily volatility, downside volatility, amount, and sector-relative strength. | **Priority 3.** Select low-volatility, low-gap, drawdown-controlled leaders in strong sectors during non-acceleration states; do not run it during climax. |
| 52-week-high scarcity | [Shanghai A-share 52-week-high evidence](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=6403338) reports 0.28% mean monthly return over 1995–2018; [AH52 evidence](https://doi.org/10.1016/j.jbankfin.2022.106531) finds the proportion of stocks near their 52-week highs predicts momentum profits. | The feature layer already has 52-week position; market and sector AH52 can represent a lower-resistance path. | **Priority 4.** Gate first on market and sector state, then select names near new highs but not in extreme acceleration. This is not raw new-high chasing and 0.28% is not a long-only promise. |
| T+1 high-turnover decline repair | [Yao and Yang](https://doi.org/10.1016/j.econmod.2026.107783) use 2002–2021 data and find strong reversal after high-turnover declines, concentrated in next-day intraday returns and in larger liquid stocks. Their [replication package](https://data.mendeley.com/datasets/9tw7g6y7xs/1) is public. | Daily OHLCV can form the signal and next-day path, but a MAOQ buyer is bound by T+1 and cannot copy the paper's intraday round trip. | **Exploratory priority 5.** Study next-open entry after a high-turnover decline in the market or a strong sector, then hold at least through the following session. Validate overnight risk independently. |
| Price-limit delayed discovery | 2025 evidence finds that A-share daily momentum is dominated by next-day abnormal returns after price-limit hits and that limits delay price discovery. | Daily bars and exact limit prices identify events, but cannot reveal queue position, seal breaks, or intraday path. | **Low-priority independent trial.** Require a first limit-up, synchronous sector breadth, an openable next session, and a non-excessive gap. It must not overwrite the failed close-limit-up implementation. |

Volatility management is a shared allocator rather than another stock-selection tactic. [Chinese volatility-managed-factor evidence](https://www.cfrn.com.cn/uploads/master/file/20240423/66272aa969986.pdf) reports 1.59 out-of-sample annualized Sharpe for a multifactor portfolio, but uses 71 factors and mean-variance optimization. MAOQ borrows only the principle: scale each tactic's risk budget by its own recent realized volatility without copying the optimizer or headline number.

### Frozen implementation queue

1. `correlation_cluster_sector_rotation_v1`: first batch. Rebuild sector correlation clusters weekly; use within-cluster trend agreement, breadth, and crowding to choose momentum, repair, or cash, then map to a few executable stocks.
2. `sector_residual_strength_v1`: first batch. Jointly rank cumulative positive residual return and residual volatility after removing market and sector common returns; hold only when the sector itself qualifies.
3. `low_volatility_sector_leader_v1`: first batch. Select low-volatility leaders in strong sectors during rotation or defensive states, without competing with emotion leaders during launch/acceleration.
4. `ah52_resistance_path_v1`: second batch. Use market and sector 52-week-high participation to identify shrinking supply resistance, then choose first approaches rather than repeatedly extreme winners.
5. `t1_high_turnover_repair_v1`: second-batch exploration. Use an at-least-two-session execution path compatible with T+1 and model overnight gaps and limit-down inability to exit.
6. `first_limit_delayed_discovery_v1`: evaluate last. It first requires historically exact price-limit regimes, listing phase, and openability; sealed next opens are always `watch_only`.
7. `volatility_managed_tactic_allocator_v1`: shared risk overlay. It changes exposure and disablement state only, never stock scores, and lower exposure cannot be presented as new selection alpha.
