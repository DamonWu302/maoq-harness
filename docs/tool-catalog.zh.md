...
| `@deepseek-ai/dsh-tool-maoq-decision` | `maoq_decide` | `ctx.tools`、`ctx.workflowEngine`、`ctx.subagents`、`ctx.systemPrompt`、`a calling Agent` | `tool/call`、`tool/result`、`workflow and child session events during execution` | - | 固定议事组只运行统帅选中的专家，随后启动新的综合子 Agent 和独立风险审查者；风险否决在模型控制之外强制执行。 |
...
固定议事组只运行统帅选中的专家，随后启动新的综合子 Agent 和独立风险审查者；风险否决在模型控制之外强制执行。
