...
| `@deepseek-ai/dsh-tool-maoq-decision` | `maoq_analyze_strategy`、`maoq_decide`、`maoq_select_tactics`、`maoq_state_get`、`maoq_state_history`、`maoq_state_latest`、`maoq_state_refresh_daily` | `ctx.agents`、`ctx.tools`、`ctx.workflowEngine`、`ctx.subagents`、`ctx.systemPrompt`、`a calling Agent` | `tool/call`、`tool/result`、`workflow and child session events during execution` | - | 战略路径先计算不可变快照特征，再运行所选专家，把解释约束到证据和获准毛选方法，并在模型控制之外保留独立风险否决；诊断路径用于检验议事组运行时。 |
...
从不可变快照计算确定性市场状态、情绪周期和板块战场特征，再运行证据约束的解释与独立风险审查。该工具不能排序股票或发出订单。
...
```json
{
  ...
  "properties": {
    "objective": {
      "type": "string",
      "description": "Concrete strategic question for this snapshot."
    },
    "snapshotHash": {
      "type": "string",
      "description": "Current immutable market snapshot SHA-256 hash."
    },
    "historySnapshotHashes": {
      "type": "array",
      "description": "At least two prior snapshot hashes for sector persistence.",
      "items": {
        "type": "string"
      }
    },
    "decisionTime": {
      "type": "string",
      "description": "Explicit ISO timestamp for replayable staleness validation."
    },
    "maximumAgeHours": {
      "type": "number",
      "description": "Non-negative maximum feature age in hours."
    },
    "specialists": {
      "type": "array",
      "description": "Ordered smallest sufficient P2 specialist subset.",
      "items": {
        "type": "string",
        "enum": [
          "market_regime",
...
运行有界 MAOQ 决策议事组。为当前目标选择最小充分专家集合；工具只运行这些专家，随后启动新的综合 Agent 和独立风险审查者。风险审查者可以否决结果。输出仅限分析或模拟决策，绝不发出实盘订单。
...
生成或复用唯一的 MAOQ 每日标准战略状态。宿主选择最近三个不同交易日的快照，并固定目标、专家视角、决策时间与时效策略；模型不能改变这些输入。本工具不能排序股票或发出订单。
...
战略路径先计算不可变快照特征，再运行所选专家，把解释约束到证据和获准毛选方法，并在模型控制之外保留独立风险否决；诊断路径用于检验议事组运行时。
...
发现并持久化一个近期、通过质量门禁的不可变市场快照有界窗口。截止点与来源版本保持精确；本工具不分析市场，也不排序股票。
...
按精确 SHA-256 哈希加载并校验一个不可变市场快照，返回有界身份与质量元数据，而不是全部股票行。
...
校验并列出本地存储的不可变快照摘要及其精确哈希。结果从新到旧排列，且不会隐式选择某个快照。
...
列出已注册的不可变市场快照来源，以及各来源是否支持近期交易日发现。本工具不执行取得。
