...
| `@deepseek-ai/dsh-tool-maoq-decision` | `maoq_analyze_strategy`, `maoq_decide`, `maoq_select_tactics`, `maoq_state_get`, `maoq_state_history`, `maoq_state_latest`, `maoq_state_refresh_daily` | `ctx.agents`, `ctx.tools`, `ctx.workflowEngine`, `ctx.subagents`, `ctx.systemPrompt`, `a calling Agent` | `tool/call`, `tool/result`, `workflow and child session events during execution` | - | The strategic path computes immutable-snapshot features before selected specialists run, binds interpretation to evidence and allowlisted Mao methods, and keeps an independent risk veto outside model control; the diagnostic path exercises the council runtime. |
...
Compute deterministic market regime, emotion cycle, and sector battlefield features from immutable snapshots, then run evidence-bound interpretation and independent risk review. This tool cannot rank stocks or place orders.
...
Run a bounded MAOQ decision council. Select the smallest sufficient specialist set for the current objective; the tool runs only those specialists, then a fresh synthesis agent and a separate risk reviewer. The risk reviewer can veto the result. Output is analysis or a paper decision only and never places a live order.
...
Read one full persisted MAOQ strategic decision mirror by decision ID without starting agents. Always inspect freshness.currentUseAllowed before using it as a current decision.
...
List recent persisted MAOQ strategic state summaries without starting agents. Use this for multi-day review and trend questions.
...
Read the newest persisted MAOQ strategic decision mirror without starting agents or recomputing market data. Always inspect freshness.currentUseAllowed before using it as a current decision.
...
Generate or reuse the one canonical daily MAOQ strategic state. The host selects the newest three distinct trading-day snapshots and fixes the objective, specialist lenses, decision time, and age policy; the model cannot vary them. This tool cannot rank stocks or place orders.
