...
生产日线在上海时区 19:00 开始自动更新。因此 MAOQ 在 19:15 才进行首次正式快照检查，此后在两小时修订窗口内每 15 分钟只重试低成本身份检查，并且仅在可用的当日不可变哈希出现后启动战略 Agent。19:00 的定时器绝不能假设更新已经完成。
...
首轮正式基线固定在 2022—2025 的 969 个四表逐股票完全一致交易日。2026 年早期历史参考表与价格表的股票集合尚不一致，不能为了扩大样本静默并入；2026-08-11 之后的连续生产数据用于日常探针和未来封存留出集。
