...
`evaluateTacticEligibility()` 消费一份 `StrategicFeatureRecord`。主动战法首先要求市场状态、情绪周期和板块战场组件均已就绪。随后，每项目录定义拥有自己的市场状态与情绪周期覆盖，并应用排名第一板块得分为正的门禁。自动化覆盖要求每种市场状态至少存在一个主动战法家族。通过后最多暴露三个正得分板块 ID；失败结果携带稳定原因码和准确的 P2 证据引用。
