...
`generateResearchTacticSignal()` 实现六项固定 P3 试验。首批三项覆盖状态签名突破／回踩、可成交情绪龙头和行业相对修复。第二轮先合并 20 交易时段相关性不低于 0.75 的板块再对领先簇排名，只在合格板块内对个股正残差排名，并在非加速状态选择低波领涨。第二轮试验每五个交易时段接受一次新入场；所有阈值与组合设置均带版本，模型不能调整。
