...
**Status:** in progress. The registry and eligibility path, point-in-time daily-history features, shared A-share next-open execution, content-addressed read-only MySQL history adapter, ten fixed research trials, and the P3.5 shared tactic catalog, conditional scorecard, deterministic top-three quantitative recommendation, complete hard-feasible advisory universe, and model-led tactic council are implemented. Production history spans 969 strictly quality-approved sessions from 2022 through 2025; the deterministic dynamic route returns -3.14% after switching costs versus 9.26% for SSE Composite, and it is active on only 38 of 301 `risk_on_trend` observations. Historical model-decision coverage is zero. These results reject promotion of the current router and identify excessive bull-state abstention, but cannot measure incremental value from DSH synthesis. The [dynamic tactic commander](maoq-dynamic-tactic-commander.md) owns phase status, attribution limits, and next-version constraints; see [MAOQ P3 tactic research](maoq-p3-tactic-research.md) for the six previously evaluated trials and the four newly preregistered trials that still require sealed evaluation.
...
The dynamic catalog contains ten distinct active opportunity families plus `defensive_no_trade`; it does not treat nearby parameter variants as new market coverage. P3.5 conditions matured tactic evidence on the state available at each cutoff and produces a deterministic top-three quantitative recommendation. The DSH planner dynamically selects two experts, and the commander may choose at most one primary and one secondary tactic from the complete hard-feasible advisory universe under host validation and the independent risk veto. Selecting outside the quantitative top three cannot create paper authority.
