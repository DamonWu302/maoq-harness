...
**Status:** in progress. The registry and eligibility path, point-in-time daily-history features, shared A-share next-open execution, content-addressed read-only MySQL history adapter, six fixed research trials, and the P3.5 shared tactic catalog, conditional scorecard, deterministic top-three router, and constrained commander path are implemented. Production history spans 969 strictly quality-approved sessions from 2022 through 2025; the deterministic dynamic route returns -2.43% after switching costs and does not beat no-trade, while historical model-decision coverage is zero. These results reject promotion of the current router but cannot measure incremental value from DSH synthesis. The [dynamic tactic commander](maoq-dynamic-tactic-commander.md) owns phase status, attribution limits, and next-version constraints; see [MAOQ P3 tactic research](maoq-p3-tactic-research.md) for fixed-trial evidence.
